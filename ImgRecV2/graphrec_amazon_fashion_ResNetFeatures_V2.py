...
#linear activation no image embedding layer
print('*************** Fashion Data ResNet Features  vs DVBPR LR 0.0001 lambda 0.1 linear activation img reg 0.00001 *************************')
#!python3 --version
############################      Read Data     #####################################
...
Features_Size=2048
reg_lambda=0.1 # was 0.005
Img_reg_lambda=0.00001
###############################################################################
...
#***************************************************************************************************
############ image embedding #############################
image_features_embedding=tf.compat.v1.layers.dense(images_features,units=image_embedding,activation=tf.nn.relu,kernel_regularizer=tf.keras.regularizers.l2(Img_reg_lambda)) # was crelu,relu (best),swish,leaky_relu
#*****************************************************************
items_with_images=tf.concat([encoded_items_onehot,image_features_embedding],axis=1)
#*****************************************************************
############ user embedding #############################
user_embedding=tf.compat.v1.layers.dense(encoded_users_onehot,units=embedding_u,kernel_regularizer=tf.keras.regularizers.l2(reg_lambda))

############ item embedding #############################
item_embedding=tf.compat.v1.layers.dense(items_with_images,units=embedding_i,kernel_regularizer=tf.keras.regularizers.l2(reg_lambda))
...
with tf.compat.v1.Session() as session:
  ...
  for i in range(0,num_of_epochs):
    ...
    for j in range(0,int(Number_of_minibatches_Train)):
      ...
      #users_feed,items_feed=One_Hot_Encoding(users_feed,items_feed,num_users,num_items)
